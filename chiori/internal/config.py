...
class ChioConfig(BaseModel):
    ...
    """Нужно ли создавать модели для базы данных.

    Обычно эту операцию нужно делать один раз - при изменении моделей.
    В остальное же время нет необходимости создавать модели при запуске.
    """

    SYNC_COMMANDS: bool = True
    """Нужно ли синхронизировать команды при запуске.

    Во время запуска клиент отправил запрос на обновление списка своих команд.
    Это может быть полезно при отладке, но операция дорогая.
    При реальной работе лучше отключить эту опцию.
    """
