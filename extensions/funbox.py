...
async def dice(
    ...
    подброса каждого кубика и конечную сумму.
    """
...
async def flip_handler(
    ctx: arc.GatewayContext
):
    """Подбрасывает монетку.

    Вы можете подбросить только одну монетку.
    В рузльтате может выпаст орёл или решка.
    Сообщение с результатом содержит выпавшую сторону монетки.
    """
    result = random.randint(0, 1)
    result_str = _FLIP_RESULTS[result]
    await ctx.respond(
        f"🪙 Подбросив монетку вы увидели там {result_str}"
    )

# 8шар -------------------------------------------------------------------------

_BALL_PREFIX = [
    "соверенно точно ", "определённо ", "скорее всего ",
    "звезды подчказывают что ", "возможно ", "вероятно ",
]

_BALL_RESULT = [
    "да", "нет", "попробуйте позже", "не знаю", "не уверен"
]

@plugin.include
@arc.slash_command("ball", description="Получить совет от шара")
async def flip_handler(
    ctx: arc.GatewayContext
):
    """8 шар.
